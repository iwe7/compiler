...
export class CssParser {
  ...
  _parseAtRule(delimiters: number): CssRuleAst {
    ...
    switch (type) {
      ...
      case BlockType.Keyframes:
        tokens = this._collectUntilDelim(
      ...
      case BlockType.MediaQuery:
        ...
        strValue = this._extractSourceContent(
          start,
          this._getScannerIndex() - 1
        );
        span = this._generateSourceSpan(startToken, block);
  ...
  _parseSelectorRule(delimiters: number): CssRuleAst {
    ...
    if (block != null) {
      ...
      const name = this._extractSourceContent(
        start,
        this._getScannerIndex() - 1
      );
  ...
  _parseSelectors(delimiters: number): CssSelectorAst[] {
    ...
    while (isParsingSelectors) {
      ...
      isParsingSelectors = !characterContainsDelimiter(
        this._scanner.peek,
        delimiters
      );
  ...
  _parsePseudoSelector(delimiters: number): CssPseudoSelectorAst {
    ...
    if (this._scanner.peek == chars.$LPAREN) {
      ...
      if (_pseudoSelectorSupportsInnerSelectors(pseudoSelectorName)) {
        let innerDelims =
          startingDelims | LPAREN_DELIM_FLAG | RPAREN_DELIM_FLAG;
        ...
        const innerValueDelims =
          ...
          LPAREN_DELIM_FLAG;
        while (
          !characterContainsDelimiter(this._scanner.peek, innerValueDelims)
        ) {
          const token = this._scan();
          tokens.push(token);
        }
  ...
  _parseSimpleSelector(delimiters: number): CssSimpleSelectorAst {
    ...
    let loopOverSelector = !characterContainsDelimiter(
      this._scanner.peek,
      selectorPartDelimiters
    );
    ...
    while (loopOverSelector) {
      ...
      loopOverSelector = !characterContainsDelimiter(
        this._scanner.peek,
        selectorPartDelimiters
      );
    }
    ...
    hasAttributeError =
      hasAttributeError ||
      this._scanner.getMode() == CssLexerMode.ATTRIBUTE_SELECTOR;
          ...
          switch (tokenOperator) {
            case SLASH_CHARACTER:
              ...
              if (
                ...
                const text =
                  SLASH_CHARACTER + deepToken.strValue + deepSlash.strValue;
                this._error(
                  generateErrorMessage(
                    this._getSourceContent(),
            ...
            case GT_CHARACTER:
              // >>> operator
              if (
                this._scanner.peek == chars.$GT &&
                this._scanner.peekPeek == chars.$GT
              ) {
    ...
    if (
      operator == null &&
      operatorScanCount > 0 &&
      this._scanner.peek != chars.$LBRACE
    ) {
  ...
  _collectUntilDelim(
    delimiters: number,
    assertType: CssTokenType | null = null
  ): CssToken[] {
    const tokens: CssToken[] = [];
    while (!characterContainsDelimiter(this._scanner.peek, delimiters)) {
