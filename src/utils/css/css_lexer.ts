...
export class CssScanner {
  ...
  consume(type: CssTokenType, value: string | null = null): LexedCssResult {
    ...
    this.setMode(
      _trackWhitespace(mode) ? CssLexerMode.ALL_TRACK_WS : CssLexerMode.ALL
    );
  ...
  _scan(): CssToken | null {
    ...
    if (
      _trackWhitespace(this._currentMode) &&
      (chars.isWhitespace(peek) || isNewline(peek))
    ) {
  ...
  scanComment(): CssToken | null {
    if (
      this.assertCondition(
        isCommentStart(this.peek, this.peekPeek),
  ...
  error(
    ...
    const invalidToken = new CssToken(
      index,
      column,
      line,
